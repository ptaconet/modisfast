...
Importantly, the robust, sustainable, and cost-free foundational framework of `modisfast`, both for the data provider (NASA) and the software (R, OPeNDAP, the `tidyverse`  and `GDAL` suite of packages and software [@tidyverse; @gdal]), guarantees the long-term stability and reliability of the package.
