...
log <- odr_login(c(Sys.getenv("earthdata_un"),Sys.getenv("earthdata_pw")),"earthdata")


## test that errors are working
test_that("test if errors are sent back", {
  # wrong type for roi
  expect_error(odr_get_url(collection = "MOD11A1.006", roi = "not_a_sf_object", time_range = as.Date(c("2017-01-01","2017-02-01"))),"Argument roi must be a object of class sf or sfc with only POLYGON-type geometries")
 # wrong type for time range
  expect_error(odr_get_url(collection = "MOD11A1.006", roi = roi, time_range = c("2017-01-01","2017-02-01")),"Argument time_range is not of class Date or POSIXlt or is not of length 1 or 2 \n")
  # wrong dates
  expect_error(odr_get_url(collection = "MOD11A1.006", roi = roi, time_range = as.Date(c("2000-01-01","2017-02-01"))),"First time frame in time_range argument is before the beginning of the mission\n")
  expect_error(odr_get_url(collection = "MOD11A1.006", roi = roi, time_range = as.Date(c("2000-01-01","1999-02-01"))),"Time end is superior to time start in time_range argument \n")
  # wrong length for time range
  expect_error(odr_get_url(collection = "MOD11A1.006", roi = roi, time_range = as.Date(c("2017-01-01","2017-02-01","2017-02-03"))),"Argument time_range is not of class Date or POSIXlt or is not of length 1 or 2 \n")
  # wrong type for singleNetcdf
  expect_error(odr_get_url(collection = "MOD11A1.006", roi = roi, time_range = as.Date(c("2017-01-01","2017-02-01")), single_netcdf = "TRUE"),"single_netcdf argument must be boolean\n")
...
## test that the function odr_get_url is working (above in comments, to test all the collections)
...
for (i in 1:length(collection_tested)){
  ...
  test_that(paste0(collection_tested[i]," is working"), {
    ...
    opendap_urls <- odr_get_url(collection = collection_tested[i],
                                    roi = roi,
    ...
    res <- odr_download_data(opendap_urls,source = "earthdata", parallel = TRUE)
