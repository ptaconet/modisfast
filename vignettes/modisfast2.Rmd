---
title: "2. Get data on several periods of interest at a glance"
author: "Paul Taconet"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Get data on several periods of interest at a glance}
...
So far so good, but what if we need ***multiple time frames of interest*** ? This case are likely to happen, for instance : we have spatiotemporal sampling data - e.g. species occurrence - that were collected over a large time frame and we want to study how local past environmental / climatic conditions influence the occurrence.

We could use `for` loops or related stuff to do the job. However, this would not be very optimized. In this vignette, we explain why and we show how to **optimize the data import in the case of multiple time periods over a ROI**. Let's start ! 
...
res_dl <- mf_download_data(urls_mod11a1, path = target_fold)

modis_ts <- mf_import_data(path = file.path(target_fold,"MOD11A1.061"), collection_source = "MODIS")
...
<!-- The same philosophy applies for multiple regions of interest :  -->
...
<!-- urls_mod11a1 <- purrr::map_dfr(roi, ~mf_get_url( -->
<!--         collection = "MOD11A1.061", -->
<!--         variables = c("LST_Day_1km","LST_Night_1km","QC_Day","QC_Night"), -->
<!--         roi = ., -->
