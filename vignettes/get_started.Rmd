...
- a 30 days-long time series of land surface temperature from [MODIS Terra LST](https://dx.doi.org/10.5067/MODIS/MOD11A1.061) (spatial resolution : 1 km ; temporal resolution : 1 day),
- the same 30 days-long times series of precipitations from [Global Precipitation Measurement (GPM)](https://doi.org/10.5067/GPM/IMERGDF/DAY/07) (spatial resolution : 1° ; temporal resolution : 1 day)
...
- **MOD11A1.061** : MODIS/Terra Land Surface Temperature/Emissivity Daily L3 Global 1km SIN Grid v061
...
First we prepare the script : define the ROI, the time frame, and the target data folder

```{r example_prepare, eval=T}
# Set ROI and time range of interest
roi <- st_as_sf(data.frame(geom="POLYGON ((-5.82 9.54, -5.42 9.55, -5.41 8.84, -5.81 8.84, -5.82 9.54))"), wkt="geom", crs = 4326)
time_range <- as.Date(c("2017-01-01","2017-01-30"))
target_fold <- "data_korhogo"
```
...
urls_mod11a1 <- mf_get_url(
  ...
  variables = c("LST_Day_1km","LST_Night_1km","QC_Day","QC_Night"),  # get the available variables with : mf_list_variables("MOD11A1.061"), or set to NULL (default) to download all the variables
  roi = roi,
...
urls_gpm <- mf_get_url(
  ...
  variables = c("precipitation"),  # get the available variables with : mf_list_variables("GPM_3IMERGDF.07")
  roi = roi,
  time_range = time_range )
...
# import MOD11A1.061
modis_ts <- mf_import_data(path = file.path(target_fold,"MOD11A1.061"), 
                           collection_source = "MODIS")
# import GPM
gpm_ts <- mf_import_data(path = file.path(target_fold,"GPM_3IMERGDF.07"), 
...
terra::plot(modis_ts["LST_Day_1km"])
# Precipitation
