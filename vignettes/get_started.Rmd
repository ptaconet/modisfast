...
```{r eval=T}
username <- Sys.getenv("earthdata_un") # replace with your own Earthdata username
password <- Sys.getenv("earthdata_pw")  # replace with your own Earthdata password
...
res_dl <- mf_download_data(df_to_dl,parallel = TRUE)
...
`r emo::ji("warning")` **Important warning regarding the use of the data after download through `modisfast`**
...
# import GPM
gpm_ts <- mf_import_data(file.path(roi_id,"GPM_3IMERGDF.07"), collection_source = "GPM")
