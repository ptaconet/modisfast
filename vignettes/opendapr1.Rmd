...
With the function `get_url()`, we get the https URLs for our collections of interest (*MOD11A1.006*, *GPM_L3/GPM_3IMERGDF.06* and *SMAP/SPL3SMP_E.003*) given our ROI and time frame. 

**Note on the use of the parameter `variables` of the function `get_url()`** :

The collections generally contain several variables (sometimes called "dimensions", or "bands"). As an example, MODIS LST products contain one band for the day temperature ("LST_Day_1km"), one band for the night temperature ("LST_Night_1km"), etc. To get all the variables available for a given collection along with information for each of them (description, etc.), use the function `get_variables_info()`. e.g. :

```{r get_variables_info, eval=NOT_CRAN}
tail(get_variables_info("MOD11A1.006"))
```

In `get_url()`, the parameter `variables` enables to restrict the data to download to only specific variables. By default it is set to `NULL`, which means that all the available variables for the specified collection are downloaded. Specifying `variables` will make the data to download lighter. 
...
urls_mod11a1 <- get_url(
  ...
  variables = c("LST_Day_1km","LST_Night_1km","QC_Day","QC_Night"),  # get the variables available with : get_variables_info("MOD11A1.006") ; or set to NULL (defaults) to download all the variables
  roi = roi,
...
urls_gpm <- get_url(
  ...
  variables = c("precipitationCal","precipitationCal_cnt"),  # get the variables available with : get_variables_info("GPM_L3/GPM_3IMERGDF.06")
  roi = roi,
  time_range = time_range
 )
...
urls_smap <- get_url(
  ...
  variables = c("Soil_Moisture_Retrieval_Data_AM_soil_moisture","Soil_Moisture_Retrieval_Data_AM_retrieval_qual_flag","Soil_Moisture_Retrieval_Data_PM_soil_moisture_pm","Soil_Moisture_Retrieval_Data_PM_retrieval_qual_flag_pm"),     # get the variables available with : get_variables_info("SMAP/SPL3SMP_E.003")
  roi = roi,
  time_range = time_range
 )
...
We set-up the missing bounding coordinates. For this we use the function `get_optional_parameters()`.
